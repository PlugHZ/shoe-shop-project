...
const EditProduct = () => {
    ...
    if (loading) return <h2 className="container" style={{padding: '3rem 0'}}>กำลังโหลดข้อมูลเก่า...</h2>;
        ...
        <div className="product-form-container container">
            ...
            <form onSubmit={handleSubmit} className="product-form">
                ...
                <div className="form-group">
                    ...
                    <input type="text" id="sizes" name="sizes" value={sizes} onChange={(e) => setSizes(e.target.value)} placeholder="เช่น 38,39,40,41" required />
                </div>
                    ...
                    <div className="image-preview-container existing-images">
                        {existingImages.map((url, index) => (
                            <div key={index} className="image-preview-wrapper">
                                <img src={url} alt={`Existing ${index + 1}`} className="image-preview" />
                                <button type="button" onClick={() => handleRemoveExistingImage(url)} className="remove-image-btn">
                                    X
                                </button>
                            </div>
                        ))}
                    ...
                    <div className="form-group">
                        ...
                        <div className="image-preview-container new-images">
                            {previewNewImages.map((url, index) => (
                                <img key={index} src={url} alt={`New Preview ${index + 1}`} className="image-preview" />
                            ))}
                        </div>
                    </div>
                )}
